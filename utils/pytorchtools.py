...
class EarlyStopping:
    ...
    def __call__(self, monitor_metric, model, **kwarg):
        ...
        if self.best_score is None:
            ...
            self.counter += 1
            self.trace_func(f'Valid Best, {abs(self.best_score)}')
    ...
    def save_checkpoint(self, monitor_metric, model, **kwarg):
        ...
        if self.verbose:
            self.trace_func(f'Validation {self.monitor} is improved ({abs(self.monitor_metric_min):.6f} --> {abs(monitor_metric):.6f}).  Saving model ...')
        if self.save_model:
        ...
        with open(os.path.join(self.path, "performance.json"), "w") as f:
            json.dump(kwarg["performance"], f , indent=6)
