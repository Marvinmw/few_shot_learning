...
for p in   Math JacksonCore Mockito JxPath Time
do
output_folder=results/single_${p}_fixed_contrastive_${num_class}_loss_both/context
...
for p in   Math JacksonCore Mockito JxPath Time
do
output_folder=results/single_${p}_fixed_contrastive_${num_class}_loss_CE/context
...
for p in   Math JacksonCore Mockito JxPath Time
do
output_folder=results/single_${p}_fixed_scratch_${num_class}_loss_both/context
bash mutant_train_single.sh gat "pretrained_models/context/gat/model_0" ${output_folder}/ attention $device $num_class both $p -1
done

for p in   Math JacksonCore Mockito JxPath Time
do
output_folder=results/single_${p}_fixed_scratch_${num_class}_loss_CE/context
